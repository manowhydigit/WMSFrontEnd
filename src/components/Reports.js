...
const menuCategories = [
  {
    ...
    items: [
      { name: "Stock Consolidation", backendName: "StockConsolidation" },
      { name: "Stock Batch Status", backendName: "StockBinBatchStatusWise" },
      { name: "Stock Bin Wise", backendName: "StockConsolidationBinWise" },
      { name: "Stock Ledger", backendName: "StockLedger" },

      // { name: "Buyer", backendName: "BUYER" },
      // { name: "Carrier", backendName: "CARRIER" },
      // { name: "Supplier", backendName: "SUPPLIER" },
      // { name: "Unit", backendName: "UNIT" },
    ],
  },
];

// Fixed routes mapping - use backendName as key
const routes = {
  StockConsolidation: "/StockConsolidation",
  StockBinBatchStatusWise: "/StockBinBatchStatusWise",
  StockConsolidationBinWise: "/StockConsolidationBinWise",
...
const Reports = () => {
  ...
  const handleNavigate = (menuItem) => {
    const route = routes[menuItem.backendName];
    if (route) {
      navigate(route);
    } else {
      console.error(`Route not found for: ${menuItem.backendName}`);
      // Optional: show a notification to the user
      notification.error({
        message: "Navigation Error",
        description: `Page not found for ${menuItem.name}`,
      });
    }
  };
            ...
            >
              {category.items.map((item, index) => (
                <div
                  key={index}
                  style={buttonStyles}
                  onClick={() => handleNavigate(item)} // Pass the entire item object
                >
